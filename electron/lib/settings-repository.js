...
class SettingsRepository {
    ...
    #defaultSettings = {
        settingsVersion: 1, // increment when an update makes settings incompatible with old settings
        general: {
            checkForUpdates: {
    ...
    constructor() {
        ...
        if (this.#settings.settingsVersion !== this.#defaultSettings.settingsVersion) { // don't use old settings format
    ...
    #validateSettingAccess(settingArea, settingName) {
        ...
        if (!settingAreas.includes(settingArea)) {
            throw new Error(
                `Setting Area ${settingArea} does not exist in the setting object. Available areas: ${JSON.stringify(
                    settingAreas
                )}`
            );
        }
        ...
        if (!settingItems.includes(settingName)) {
            throw new Error(
                `Setting Item ${settingName} does not exist in the ${settingArea} setting area. Available setting items: ${JSON.stringify(
                    settingItems
                )}`
            );
        }
    ...
    #refreshSettings() {
        try {
            ...
            fs.writeFileSync(SETTINGS_FILE_LOCATION, JSON.stringify(this.#defaultSettings), 'utf8');
        }
    ...
    getAreas() {
        return Object.keys(this.#settings).filter(key => key !=='settingsVersion');
    }
    ...
    update({ settingArea, settingName, newValue }) {
        ...
        if (
            ...
            throw new Error(
                `The attempted value ${newValue} is not valid. Available options: ${JSON.stringify(
                    this.#settings[settingArea][settingName].options
                )}`
            );
        }
